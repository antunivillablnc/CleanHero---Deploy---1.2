...
export default function RewardsPage() {
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto">
      <h1 className="text-3xl font-semibold mb-6 text-gray-800 dark:text-white">Rewards</h1>
      
      <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-lg flex flex-col justify-between h-full border-l-4 border-green-500 mb-8">
        <h2 className="text-xl font-semibold mb-4 text-gray-800 dark:text-gray-100">Reward Balance</h2>
        ...
        <div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-md overflow-hidden">
            {transactions.length > 0 ? (
              ...
              <div className="p-4 text-center text-gray-500 dark:text-gray-400">No transactions yet</div>
            )}
          </div>
        </div>

        <div>
          <h2 className="text-2xl font-semibold mb-4 text-gray-800 dark:text-white">Available Rewards</h2>
          <div className="space-y-4">
            {rewards.length > 0 ? (
              rewards.map(reward => (
                <div key={reward.id} className="bg-white dark:bg-gray-800 p-4 rounded-xl shadow-md">
                  <div className="flex justify-between items-center mb-2">
                    <h3 className="text-lg font-semibold text-gray-800 dark:text-gray-100">{reward.name}</h3>
                    <span className="text-green-500 font-semibold">{reward.cost} points</span>
                  </div>
                  <p className="text-gray-600 dark:text-gray-300 mb-2">{reward.description}</p>
                  <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">{reward.collectionInfo}</p>
                  {reward.id === 0 ? (
              ...
              <div className="bg-yellow-50 dark:bg-yellow-900/30 border-l-4 border-yellow-400 p-4 rounded-md">
                <div className="flex items-center">
                  ...
                  <p className="text-yellow-700 dark:text-yellow-400">No rewards available at the moment.</p>
                </div>
              </div>
            )}
          </div>
