...
export async function createUser(email: string, name: string) {
  try {
    ...
    const adminEmails = ['adriancamota@gmail.com'];
...
export async function updateRewardPoints(userId: number, pointsToAdd: number) {
  try {
    // First get current reward to calculate new level
    const currentReward = await getOrCreateReward(userId);
    if (!currentReward) {
      throw new Error("Failed to get current reward");
    }

    // Calculate new total points and level
    const newTotalPoints = currentReward.points + pointsToAdd;
    ...
    // If level increased, create a notification
    if (newLevel > currentReward.level) {
      await createNotification(
        userId,
...
export async function getAllRewards() {
  try {
    ...
    // Calculate total points and determine level for each user
    const userRewards = users.map(user => {
      const userTransactions = transactions.filter(t => t.userId === user.id);
      const totalPoints = userTransactions.reduce((acc, transaction) => {
        return transaction.type.startsWith('earned') 
          ? acc + transaction.amount 
          : acc - transaction.amount;
      }, 0);

      // Calculate level based on total points (1 level per 100 points)
      const level = Math.floor(totalPoints / 100) + 1;

      return {
        id: user.id,
        userId: user.id,
        points: Math.max(totalPoints, 0), // Ensure points don't go negative
        level,
        createdAt: new Date(), // This is less relevant now but kept for interface compatibility
        userName: user.name,
      };
    });
...
export async function getImpactData() {
  try {
    ...
    const wasteCollected = reports.reduce((total, report) => {
      const match = report.amount.match(/(\d+(\.\d+)?)/);
      const amount = match ? parseFloat(match[0]) : 0;
      return total + amount;
    }, 0);

    // Calculate total reports submitted
    const reportsSubmitted = reports.length;

    // Calculate total tokens earned (only count 'earned' transactions)
    const tokensEarned = transactions.reduce((total, transaction) => {
      return transaction.type.startsWith('earned') ? total + transaction.amount : total;
    }, 0);

    // Calculate CO2 offset (assuming 0.5 kg CO2 per kg of waste)
